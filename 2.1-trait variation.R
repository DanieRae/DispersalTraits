...
ggplot(data = fish.traits.40NA, aes(x= DepthRangeComDeep))+
  geom_histogram()

ggplot(data = fish.traits.40NA, aes(x= DepthRangeComShallow))+
  geom_histogram()
...
#Trying to make a stacked bar plot FAIL#
...
# Stacked + percent
ggplot(fish.traits.40.cat.long, aes(fill=taxa_name, y=count, x=Trait)) + 
